...
class HospitalGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("MediCare Hospital")
    ...
    def create_gui(self):
        ...
        Buttonframe.place(x=0, y=425, width=1270, height=70)

        # --------------- Details frame -----------------------------

        self.Detailsframe = Frame(self.root, bd=9, relief=RIDGE)
        self.Detailsframe.place(x=0, y=500, width=1270, height=200)
        ...
        Button(
            ...
            font=("Arial", 10, "bold"),
            width=18,
            command=lambda: self.switch_form("facility"),
        ).pack(side=LEFT, padx=5, pady=5)
    ...
    def create_facility_tree(self):
        self.facility_tree = ttk.Treeview(
            self.facility_tab,
            columns=("ID", "Name", "Type", "Premium", "Description", "Cost"),
            show="headings",
        )
        for col in self.facility_tree["columns"]:
            self.facility_tree.heading(col, text=col)
        for col, w in zip(self.facility_tree["columns"], [50, 150, 100, 80, 200, 80]):
            self.facility_tree.column(col, width=w)
        scrollbar = ttk.Scrollbar(
            self.facility_tab, orient="vertical", command=self.facility_tree.yview
        )
        self.facility_tree.configure(yscrollcommand=scrollbar.set)
        scrollbar.pack(side="right", fill="y")
        self.facility_tree.pack(expand=True, fill=BOTH)
    ...
    def create_facility_form(self):
        self.clear_form()
        self.current_form = "facility"
        self.Dataframeleft.config(text="Facility Information")

        self.facility_name = StringVar()
        self.facility_type = StringVar()
        self.facility_premium = BooleanVar()
        self.facility_description = StringVar()
        self.facility_cost = StringVar()

        labels = ["Facility Name", "Type", "Description", "Cost"]
        variables = [
            self.facility_name,
            self.facility_type,
        ...
        Checkbutton(
            self.Dataframeleft,
            text="Premium",
            variable=self.facility_premium,
            font=("poppins", 10, "bold"),
    ...
    def switch_form(self, form_type):
        ...
        elif form_type == "facility":
            self.create_facility_form()

    def submit_patient(self):
        try:
            query = "INSERT INTO patients (name, age, gender, disease) VALUES (?, ?, ?, ?)"
            values = (
    ...
    def submit_facility(self):
        try:
            query = "INSERT INTO facilities (name, type, premium, description, cost) VALUES (?, ?, ?, ?, ?)"
            values = (
                self.facility_name.get(),
                self.facility_type.get(),
                1 if self.facility_premium.get() else 0,
                self.facility_description.get(),
                float(self.facility_cost.get()) if self.facility_cost.get() else 0.0,
            )
            self.cursor.execute(query, values)
            self.conn.commit()
            messagebox.showinfo("Success", "Facility added successfully")
            self.clear_facility_fields()
            self.show_facilities()
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def show_facilities(self):
        try:
            ...
            for facility in self.cursor.fetchall():
                self.facility_tree.insert("", "end", values=facility)
            self.notebook.select(self.facility_tab)
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def clear_facility_fields(self):
        self.facility_name.set("")
        self.facility_type.set("")
        self.facility_premium.set(False)
        self.facility_description.set("")
        self.facility_cost.set("")
