...
def menu():
    ...
    while True:
        ...
        print("8. Add Facility")
        print("9. Show Facilities")
        print("10. Exit")
        ...
        if choice == "1":
            ...
            ftype = input("Facility Type: ")
            premium = input("Is Premium (yes/no): ").lower() == 'yes'
            description = input("Description: ")
            cost = float(input("Cost: "))
            f = Facility(name, ftype, premium, description, cost)
            f.save_to_db(cursor, conn)
            print("Facility added successfully.")

        elif choice == "9":
            facilities = Facility.get_all(cursor)
            if facilities:
                print("Facilities:")
                for fac in facilities:
                    print(f"ID: {fac[0]}, Name: {fac[1]}, Type: {fac[2]}, Premium: {'Yes' if fac[3] else 'No'}, Description: {fac[4]}, Cost: {fac[5]}")
            else:
